...
#2
* Immutable. 
* * throughout the program the data/value of blank does not change. It always remains as x=1 & y=2. 
```
...
```text
 #5
 The return values differ because #4 returns a Type `double` value and #3 returns an instance(Type Point). 
```
...
```text
 #3
 p1 and p2 are not aliased. Aliasing are two or more references pointing to the same object. 
 p1 and p2 are referring to different Point objects, from the findCenter method.
```
...
3. Generalize the code so that it works on any string.
    ...
    * * At this point I believe I've reached a generalized program. But a step I implented was to use a general String, in this case the one provided, as a parameter in the new `experiment1` method. 
